...
namespace TSMapEditor
{
    public static class Constants
    {
        ...
        public static void Init(bool forceReload = false)
        {
            ...
            // 根据当前游戏版本设置正确的可执行文件名
            string gameVersion = UserSettings.Instance?.GameVersion?.GetValue() ?? "Yuri's Revenge";
            if (gameVersion == "MentalOmegaClient")
            {
                ExpectedClientExecutableNames = new string[] { "MentalOmegaClient.exe" };
            }
            else // "Yuri's Revenge"
            {
                ExpectedClientExecutableNames = new string[] { "ra2md.exe", "gamemd.exe" };
            }
            
            Rampastring.Tools.Logger.Log($"已为游戏版本 {gameVersion} 重新初始化常量，可执行文件名: {string.Join(", ", ExpectedClientExecutableNames)}");
        }
