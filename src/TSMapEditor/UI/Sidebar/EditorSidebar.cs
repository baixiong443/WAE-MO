...
namespace TSMapEditor.UI.Sidebar
{
    public class EditorSidebar : EditorPanel
    {
        ...
        private static readonly List<string> sidebarModeNamesCN = new List<string>
        {
            "建筑物", "步兵", "载具", "飞行器", "海军", "地形物体", "覆盖物", "污点"
        };
        private static readonly List<string> sidebarModeNamesEN = new List<string>
        {
            "Buildings", "Infantry", "Vehicles", "Aircraft", "Naval", "Terrain Objects", "Overlays", "Smudges"
        };
        private List<string> sidebarModeNames => TSMapEditor.UI.MainMenu.IsChinese ? sidebarModeNamesCN : sidebarModeNamesEN;
        ...
        {
            for (int i = 0; i < lbSelection.Items.Count; i++)
                lbSelection.Items[i].Text = isChinese ? sidebarModeNamesCN[i] : sidebarModeNamesEN[i];
            ...
            if (modePanels != null)
            {
                foreach (var panel in modePanels)
                {
                    if (panel is TSMapEditor.UI.Sidebar.ObjectListPanel objPanel)
                        objPanel.RefreshLanguage(isChinese);
                    else if (panel is TSMapEditor.UI.Sidebar.OverlayListPanel overlayPanel)
                        overlayPanel.RefreshLanguage(isChinese);
                    else if (panel is TSMapEditor.UI.Sidebar.TerrainObjectListPanel terrainPanel)
                        terrainPanel.RefreshLanguage(isChinese);
                    else if (panel is TSMapEditor.UI.Sidebar.SmudgeListPanel smudgePanel)
                        smudgePanel.RefreshLanguage(isChinese);
                    // 其它需要多语言的面板可继续补充
                }
            }
        }
