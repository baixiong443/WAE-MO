...
namespace TSMapEditor.UI.Windows
{
    ...
    public class TriggersWindow : INItializableWindow
    {
        ...
        public static readonly List<string> UniversalHouseNames = GenerateUniversalHouseNames();
        ...
        private EditorContextMenu triggerContextMenu;
        ...
        public override void Initialize()
        {
            ...
            triggerContextMenu = new EditorContextMenu(WindowManager);
            triggerContextMenu.Name = nameof(triggerContextMenu);
            triggerContextMenu.Width = 270;
            AddChild(triggerContextMenu);

            var btnNewTrigger = FindChild<EditorButton>("btnNewTrigger");
            btnNewTrigger.LeftClick += BtnNewTrigger_LeftClick;

            var btnDeleteTrigger = FindChild<EditorButton>("btnDeleteTrigger");
            btnDeleteTrigger.LeftClick += BtnDeleteTrigger_LeftClick;

            var btnCloneTrigger = FindChild<EditorButton>("btnCloneTrigger");
            btnCloneTrigger.LeftClick += BtnCloneTrigger_LeftClick;
            ...
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按ID排序" : "Sort by ID", () => TriggerSortMode = TriggerSortMode.ID);
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按名称排序" : "Sort by Name", () => TriggerSortMode = TriggerSortMode.Name);
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按颜色排序" : "Sort by Color", () => TriggerSortMode = TriggerSortMode.Color);
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按颜色排序，然后按名称排序" : "Sort by Color, then by Name", () => TriggerSortMode = TriggerSortMode.ColorThenName);
        ...
        {
            FindChild<XNALabel>("lblTriggers").Text = MainMenu.IsChinese ? "触发器:" : "Triggers:";
            var btnNewTrigger = FindChild<EditorButton>("btnNewTrigger");
            btnNewTrigger.Text = MainMenu.IsChinese ? "新建" : "New";
            var btnDeleteTrigger = FindChild<EditorButton>("btnDeleteTrigger");
            btnDeleteTrigger.Text = MainMenu.IsChinese ? "删除" : "Delete";
            var btnCloneTrigger = FindChild<EditorButton>("btnCloneTrigger");
            btnCloneTrigger.Text = MainMenu.IsChinese ? "克隆" : "Clone";

            // 更新排序菜单文本
            var sortContextMenu = Children.OfType<EditorContextMenu>().FirstOrDefault(m => m.Name == "sortContextMenu");
            if (sortContextMenu != null && sortContextMenu.Items.Count >= 4)
            {
                ...
                sortContextMenu.Items[3].Text = MainMenu.IsChinese ? "按颜色排序，然后按名称排序" : "Sort by Color, then by Name";
            }

            FindChild<XNALabel>("lblSelectedTrigger").Text = MainMenu.IsChinese ? "当前触发器:" : "Selected Trigger:";
            FindChild<XNALabel>("lblName").Text = MainMenu.IsChinese ? "名称:" : "Name:";
            FindChild<XNALabel>("lblHouse").Text = MainMenu.IsChinese ? "势力:" : "House:";
            FindChild<XNALabel>("lblType").Text = MainMenu.IsChinese ? "类型:" : "Type:";
            FindChild<XNALabel>("lblAttachedTrigger").Text = MainMenu.IsChinese ? "附加触发器:" : "Attached trigger:";
            FindChild<XNALabel>("lblTriggerColor").Text = MainMenu.IsChinese ? "颜色:" : "Color:";
            
            // 添加参数标签的翻译
            FindChild<XNALabel>("lblActionParameters").Text = MainMenu.IsChinese ? "动作参数:" : "Action parameters:";
            FindChild<XNALabel>("lblActionParameterValue").Text = MainMenu.IsChinese ? "参数值:" : "Parameter value:";
            FindChild<XNALabel>("lblEventParameters").Text = MainMenu.IsChinese ? "事件参数:" : "Event parameters:";
            FindChild<XNALabel>("lblEventParameterValue").Text = MainMenu.IsChinese ? "参数值:" : "Parameter value:";
            
            // 添加类型标签的翻译
            FindChild<XNALabel>("lblActionType").Text = MainMenu.IsChinese ? "动作类型:" : "Action type:";
            FindChild<XNALabel>("lblEventType").Text = MainMenu.IsChinese ? "事件类型:" : "Event type:";

            // 重新加载颜色下拉菜单，支持中英文切换
            ddTriggerColor.Items.Clear();
            ddTriggerColor.AddItem(MainMenu.IsChinese ? "无" : "None");
            Array.ForEach(Trigger.SupportedColors, sc => 
            {
                string colorName = GetChineseColorName(sc.Name);
                ddTriggerColor.AddItem(colorName, sc.Value);
            });

            if (editedTrigger != null && editedTrigger.EditorColor != null)
            {
                string colorName = editedTrigger.EditorColor;
                int colorIndex = -1;
                
                if (MainMenu.IsChinese)
                {
                    // 如果当前是中文模式，需要找到对应的英文颜色的索引
                    colorIndex = Array.FindIndex(Trigger.SupportedColors, c => c.Name == colorName);
                    if (colorIndex >= 0)
                    {
                        // 索引加1是因为第一个选项是"无"
                        ddTriggerColor.SelectedIndex = colorIndex + 1;
                    }
                }
                else
                {
                    // 英文模式直接匹配文本
                    ddTriggerColor.SelectedIndex = ddTriggerColor.Items.FindIndex(item => item.Text == colorName);
                }
            ...
            // 添加对禁用复选框和难度级别标签的中文支持
            chkDisabled.Text = MainMenu.IsChinese ? "禁用(必须由另一个触发器启用才能触发)" : "Disabled (must be enabled by another trigger for this trigger to fire)";
            FindChild<XNALabel>("lblDifficulties").Text = MainMenu.IsChinese ? "在以下难度级别启用:" : "Enabled on difficulty levels:";
            chkEasy.Text = MainMenu.IsChinese ? "简单" : "Easy";
            chkMedium.Text = MainMenu.IsChinese ? "中等" : "Medium";
            chkHard.Text = MainMenu.IsChinese ? "困难" : "Hard";

            ddType.Items.Clear();
            ddType.AddItem(MainMenu.IsChinese ? "0 - 一次性，单体条件" : "0 - one-time, single-object condition");
            ddType.AddItem(MainMenu.IsChinese ? "1 - 一次性，多体条件" : "1 - one-time, multi-object condition");
            ddType.AddItem(MainMenu.IsChinese ? "2 - 重复性，单体条件" : "2 - repeating, single-object condition");

            triggerContextMenu.ClearItems();
            triggerContextMenu.AddItem(MainMenu.IsChinese ? "放置信标格" : "Place CellTag", PlaceCellTag);
            triggerContextMenu.AddItem(MainMenu.IsChinese ? "清除信标格" : "Clear CellTags", ClearCellTags);
            triggerContextMenu.AddItem(MainMenu.IsChinese ? "附加到物件" : "Attach to Objects", AttachTagToObjects);
            triggerContextMenu.AddItem(MainMenu.IsChinese ? "查看引用" : "View References", ShowReferences);
            if (!Constants.IsRA2YR)
            {
                triggerContextMenu.AddItem(MainMenu.IsChinese ? "包裹EVA语音禁用/启用" : "Wrap in EVA disable/enable actions", WrapInEVADisableAndEnableActions);
            }
            triggerContextMenu.AddItem(MainMenu.IsChinese ? "为低难度克隆" : "Clone for Easier Diffs", CloneForEasierDifficulties);
            triggerContextMenu.AddItem(MainMenu.IsChinese ? "为低难度克隆 (无依赖)" : "Clone for Easier Diffs (No Dependencies)", CloneForEasierDifficultiesWithoutDependencies);

            ddActions.Items.Clear();
            ddActions.AddItem(MainMenu.IsChinese ? "高级..." : "Advanced...");
            ...
            ddActions.AddItem(new XNADropDownItem() { Text = MainMenu.IsChinese ? "重新生成触发器ID" : "Re-generate Trigger IDs", Tag = new Action(RegenerateIDs) });

            ddActions.SelectedIndex = 0;

            // 更新选择触发器窗口的文本
            if (selectTriggerWindow != null)
                selectTriggerWindow.RefreshLanguage();
                
            // 更新选择事件窗口的文本
            if (selectEventWindow != null)
                selectEventWindow.RefreshLanguage();
                
            // 更新选择动作窗口的文本
            if (selectActionWindow != null)
                selectActionWindow.RefreshLanguage();

            // Preserve and restore selection after refreshing houses
            string currentHouse = editedTrigger?.HouseType;
            RefreshHouses();
            if (currentHouse != null)
            {
                HouseType houseType = map.GetHouseTypes().Find(h => h.ININame == currentHouse);
            ...
            FindChild<XNALabel>("lblEvents").Text = MainMenu.IsChinese ? "选定触发器的事件:" : "Events of selected trigger:";
            
            // 添加事件相关按钮的翻译
            FindChild<EditorButton>("btnAddEvent").Text = MainMenu.IsChinese ? "添加事件" : "Add Event";
            FindChild<EditorButton>("btnDeleteEvent").Text = MainMenu.IsChinese ? "删除事件" : "Delete Event";
            FindChild<EditorButton>("btnCloneEvent").Text = MainMenu.IsChinese ? "克隆事件" : "Clone Event";
            ...
            // 添加动作相关标签和按钮的翻译
            FindChild<XNALabel>("lblActions").Text = MainMenu.IsChinese ? "选定触发器的动作:" : "Actions of selected trigger:";
            FindChild<EditorButton>("btnAddAction").Text = MainMenu.IsChinese ? "添加动作" : "Add Action";
            FindChild<EditorButton>("btnDeleteAction").Text = MainMenu.IsChinese ? "删除动作" : "Delete Action";
            FindChild<EditorButton>("btnCloneAction").Text = MainMenu.IsChinese ? "克隆动作" : "Clone Action";
            
            // 刷新事件列表和动作列表，确保显示对应语言的文本
            if (editedTrigger != null)
            {
                // 保存当前选中的事件索引和动作索引
                int selectedEventIndex = lbEvents.SelectedIndex;
                int selectedActionIndex = lbActions.SelectedIndex;
                ...
                // 恢复事件选中状态
                if (selectedEventIndex >= 0 && selectedEventIndex < lbEvents.Items.Count)
                {
                    lbEvents.SelectedIndex = selectedEventIndex;
                }
                else if (lbEvents.Items.Count > 0)
                {
                    lbEvents.SelectedIndex = 0;
                }
                
                // 恢复动作选中状态
                if (selectedActionIndex >= 0 && selectedActionIndex < lbActions.Items.Count)
                {
                    lbActions.SelectedIndex = selectedActionIndex;
                }
                else if (lbActions.Items.Count > 0)
                {
        ...
        private void BtnDeleteTrigger_LeftClick(object sender, EventArgs e)
        {
            ...
            else
            {
                var msgBox = EditorMessageBox.Show(WindowManager,
                    MainMenu.IsChinese ? "您确定吗？" : "Are you sure?",
                    string.Format(MainMenu.IsChinese ? "您真的要删除触发器 \"{0}\" 吗？" : "Do you really want to delete trigger \"{0}\"?", editedTrigger.Name) + Environment.NewLine + Environment.NewLine +
                    (MainMenu.IsChinese ? "（您可以按住Shift键跳过此确认对话框。）" : "(You can hold Shift to skip this confirmation dialog.)"), MessageBoxButtons.YesNo);
        ...
        private void EditTrigger(Trigger trigger)
        {
            ...
            HouseType houseType = map.GetHouseTypes().Find(h => h.ININame == trigger.HouseType);
            if (houseType != null && ShouldDisplayHouse(houseType))
            {
                ddHouseType.SelectedIndex = ddHouseType.Items.FindIndex(item => item.Text == GetHouseDisplayName(houseType));
            }
        ...
        private void DdTriggerColor_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            // 获取英文颜色名称
            string colorName = ddTriggerColor.SelectedItem.Text;
            if (MainMenu.IsChinese)
            {
                // 在中文模式下，需要转换回英文颜色名
                int colorIndex = ddTriggerColor.SelectedIndex - 1; // 减1是因为第一个选项是"无"
                if (colorIndex >= 0 && colorIndex < Trigger.SupportedColors.Length)
                {
                    colorName = Trigger.SupportedColors[colorIndex].Name;
                }
            }

            editedTrigger.EditorColor = colorName;
        ...
        {
            if (ddHouseType.SelectedItem == null)
                return;

            string selectedHouseDisplayName = ddHouseType.SelectedItem.Text;
            HouseType selectedHouseType = map.GetHouseTypes().Find(ht =>
            {
                if (ShouldDisplayHouse(ht))
                {
                    return GetHouseDisplayName(ht) == selectedHouseDisplayName;
                }
                return false;
            });

            if (selectedHouseType != null)
                editedTrigger.HouseType = selectedHouseType.ININame;
        }
        ...
        private void LbActions_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (triggerActionType == null)
            {
                selActionType.Text = triggerAction.ActionIndex + " Unknown";
                panelActionDescription.Text = "Unknown action. It has most likely been added with another editor.";
            }
            else
            {
                // 支持中英文切换动作名称和描述
                ...
                string actionDescription = MainMenu.IsChinese && !string.IsNullOrEmpty(triggerActionType.ChineseDescription) 
                    ? triggerActionType.ChineseDescription 
                    : triggerActionType.Description;
                    
                selActionType.Text = triggerAction.ActionIndex + " " + actionName;
                panelActionDescription.Text = actionDescription;
            }
            ...
            else
            {
                for (int i = 0; i < triggerActionType.Parameters.Length; i++)
                {
                    ...
                    string paramText = TriggerParamTypeExtensions.GetLocalizedParamName(param.NameOverride, param.TriggerParamType);
                    lbActionParameters.AddItem(new XNAListBoxItem() { Text = paramText, Tag = i });
                }
            }
        ...
        private void AddAction(TriggerAction action)
        {
            ...
            lbActions.AddItem(new XNAListBoxItem() { Text = action.ActionIndex + " " + actionName, Tag = action });
        }
        ...
        private void LbEvents_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (triggerEventType == null)
            {
                selEventType.Text = triggerCondition.ConditionIndex + " Unknown";
                panelEventDescription.Text = "Unknown event. It has most likely been added with another editor.";
            }
            else
            {
                // 支持中英文切换
                string eventName = MainMenu.IsChinese && !string.IsNullOrEmpty(triggerEventType.ChineseName) 
                    ? triggerEventType.ChineseName 
                    : triggerEventType.Name;
                    
                string eventDescription = MainMenu.IsChinese && !string.IsNullOrEmpty(triggerEventType.ChineseDescription) 
                    ? triggerEventType.ChineseDescription 
                    : triggerEventType.Description;
                    
                selEventType.Text = triggerCondition.ConditionIndex + " " + eventName;
                panelEventDescription.Text = eventDescription;
            }
            ...
            else
            {
                for (int i = 0; i < triggerEventType.Parameters.Length; i++)
                {
                    ...
                    string paramText = TriggerParamTypeExtensions.GetLocalizedParamName(param.NameOverride, param.TriggerParamType);
                    lbEventParameters.AddItem(new XNAListBoxItem() { Text = paramText, Tag = i });
                }
            }
        ...
        private void AddEvent(TriggerCondition condition)
        {
            ...
            lbEvents.AddItem(new XNAListBoxItem() { Text = condition.ConditionIndex + " " + eventName, Tag = condition });
        }
        ...
        // 获取颜色的中文名称
        private string GetChineseColorName(string englishName)
        {
            if (!MainMenu.IsChinese)
                return englishName;
            ...
            switch (englishName)
            {
                ...
                case "Lime Green": return "淡绿色";
                case "Yellow": return "黄色";
                case "Orange": return "橙色";
                case "Red": return "红色";
                case "Blood Red": return "深红色";
                case "Pink": return "粉色";
