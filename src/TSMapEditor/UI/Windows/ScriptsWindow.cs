...
namespace TSMapEditor.UI.Windows
{
    ...
    public class ScriptsWindow : INItializableWindow
    {
        ...
        public override void Initialize()
        {
            ...
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按ID排序" : "Sort by ID", () => ScriptSortMode = ScriptSortMode.ID);
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按名称排序" : "Sort by Name", () => ScriptSortMode = ScriptSortMode.Name);
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按颜色排序" : "Sort by Color", () => ScriptSortMode = ScriptSortMode.Color);
            sortContextMenu.AddItem(MainMenu.IsChinese ? "按颜色排序，然后按名称排序" : "Sort by Color, then by Name", () => ScriptSortMode = ScriptSortMode.ColorThenName);
        ...
        private string GetActionEntryText(int index, ScriptActionEntry entry)
        {
            ...
            return "#" + index + " - " + (MainMenu.IsChinese ? action.ChineseName : action.Name) + " (" + entry.Argument.ToString(CultureInfo.InvariantCulture) + ")";
        }
        ...
        private string GetActionNameFromIndex(int index)
        {
            ...
            return index + " " + (MainMenu.IsChinese ? action.ChineseName : action.Name);
        }
        ...
        private string GetActionDescriptionFromIndex(int index)
        {
            ...
            string description = action == null ? 
                (MainMenu.IsChinese ? "未知脚本动作。它很可能是使用其他编辑器添加的。" : "Unknown script action. It has most likely been added with another editor.") : 
                (MainMenu.IsChinese ? action.ChineseDescription : action.Description);
        ...
        public void RefreshLanguage()
        {
            ...
            // 汉化标题和描述文本
            var lblWindowDescription = FindChild<XNALabel>("lblWindowDescription");
            if (lblWindowDescription != null)
                lblWindowDescription.Text = isChinese ? "脚本是一组供团队执行的操作。" : "Scripts are sets of actions for a team to perform.";
                
            var lblScripts = FindChild<XNALabel>("lblScriptTypes");
            if (lblScripts != null)
                lblScripts.Text = isChinese ? "脚本：" : "Scripts:";
                
            var lblSelectedScript = FindChild<XNALabel>("lblSelectedScript");
            if (lblSelectedScript != null)
                lblSelectedScript.Text = isChinese ? "已选脚本：" : "Selected Script:";
                
            // 汉化按钮
            var btnAddScript = FindChild<EditorButton>("btnAddScript");
            if (btnAddScript != null)
                btnAddScript.Text = isChinese ? "新建" : "New";
                
            var btnDeleteScript = FindChild<EditorButton>("btnDeleteScript");
            if (btnDeleteScript != null)
                btnDeleteScript.Text = isChinese ? "删除" : "Delete";
                
            var btnCloneScript = FindChild<EditorButton>("btnCloneScript");
            if (btnCloneScript != null)
                btnCloneScript.Text = isChinese ? "克隆" : "Clone";
            ...
            var lblScriptColor = FindChild<XNALabel>("lblScriptColor");
            if (lblScriptColor != null)
                lblScriptColor.Text = isChinese ? "颜色:" : "Color:";
                
            var lblActions = FindChild<XNALabel>("lblActions");
            if (lblActions != null)
                lblActions.Text = isChinese ? "动作:" : "Actions:";
                
            // 添加截图中的三个标签汉化
            var lblTypeOfAction = FindChild<XNALabel>("lblTypeOfAction");
            if (lblTypeOfAction != null)
                lblTypeOfAction.Text = isChinese ? "动作类型:" : "Type of action:";
                
            var lblParameterDescription = FindChild<XNALabel>("lblParameterDescription");
            if (lblParameterDescription != null)
                lblParameterDescription.Text = isChinese ? "参数:" : "Parameter:";
                
            var lblActionDescription = FindChild<XNALabel>("lblActionDescription");
            if (lblActionDescription != null)
                lblActionDescription.Text = isChinese ? "描述:" : "Description:";
                
            // 添加动作相关按钮的汉化
            var btnAddAction = FindChild<EditorButton>("btnAddAction");
            if (btnAddAction != null)
                btnAddAction.Text = isChinese ? "添加" : "Add";
                
            var btnMoveUp = FindChild<EditorButton>("btnMoveUp");
            if (btnMoveUp != null)
                btnMoveUp.Text = isChinese ? "上移" : "Move Up";
                
            var btnMoveDown = FindChild<EditorButton>("btnMoveDown");
            if (btnMoveDown != null)
                btnMoveDown.Text = isChinese ? "下移" : "Move Down";
                
            var btnCloneAction = FindChild<EditorButton>("btnCloneAction");
            if (btnCloneAction != null)
                btnCloneAction.Text = isChinese ? "克隆" : "Clone";
                
            var btnInsertAction = FindChild<EditorButton>("btnInsertAction");
            if (btnInsertAction != null)
                btnInsertAction.Text = isChinese ? "插入" : "Insert";
                
            var btnDeleteAction = FindChild<EditorButton>("btnDeleteAction");
            if (btnDeleteAction != null)
                btnDeleteAction.Text = isChinese ? "删除" : "Delete";
            ...
            if (sortContextMenu != null && sortContextMenu.Items.Count >= 4)
            {
                ...
                sortContextMenu.Items[3].Text = isChinese ? "按颜色排序，然后按名称排序" : "Sort by Color, then by Name";
            }
                
            // 刷新颜色下拉菜单，支持中英文切换
            // 先暂时移除事件处理器，避免触发不必要的事件
            ddScriptColor.SelectedIndexChanged -= DdScriptColor_SelectedIndexChanged;
            
            // 保存当前选中的颜色
            string currentColor = null;
            if (editedScript != null && editedScript.EditorColor != null)
                currentColor = editedScript.EditorColor;
                
            // 清空并重新添加颜色选项
            ddScriptColor.Items.Clear();
            ddScriptColor.AddItem(isChinese ? "无" : "None");
            Array.ForEach(Script.SupportedColors, supportedColor =>
            {
                string colorName = isChinese ? GetChineseColorName(supportedColor.Name) : supportedColor.Name;
                ddScriptColor.AddItem(colorName, supportedColor.Value);
            });
            ...
            if (currentColor != null)
            {
                ...
                if (isChinese)
                {
                    ...
                    colorIndex = Array.FindIndex(Script.SupportedColors, c => c.Name == currentColor);
                    if (colorIndex >= 0)
                    {
                        // 索引加1是因为第一个选项是"无"
                        ddScriptColor.SelectedIndex = colorIndex + 1;
                    }
                }
                else
                {
                    // 英文模式直接匹配文本
                    ddScriptColor.SelectedIndex = ddScriptColor.Items.FindIndex(item => item.Text == currentColor);
                }
            ...
            // 刷新脚本动作选择窗口
            if (selectScriptActionWindow != null)
                selectScriptActionWindow.RefreshLanguage();
        }
