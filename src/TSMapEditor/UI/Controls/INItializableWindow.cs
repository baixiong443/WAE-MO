...
namespace TSMapEditor.UI.Controls
{
    ...
    public class INItializableWindow : EditorWindow
    {
        ...
        public override void Initialize()
        {
            ...
            // 使用ConfigManager加载配置文件，而不是直接从固定路径加载
            string configPath = Path.Combine("UI", SubDirectory, Name + ".ini");
            ConfigIni = ConfigManager.GetConfig(configPath);
        ...
        private bool ReadINIForControl(XNAControl control, bool isForLayout = false)
        {
            ...
            foreach (var kvp in section.Keys)
            {
                ...
                else if (kvp.Key == "Text" && (control is XNALabel || control is XNACheckBox) && TSMapEditor.UI.MainMenu.IsChinese)
                {
                    // 如果当前语言是中文，尝试读取CNText属性
                    string cnText = section.GetStringValue("CNText", null);
                    if (!string.IsNullOrWhiteSpace(cnText))
                    {
                        ...
                        else if (control is XNACheckBox checkBox)
                        {
                            checkBox.Text = cnText;
                        }
                    }
                    else
                    {
            ...
            foreach (var child in children)
            {
                ...
                if (childSection != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(toolTipText))
                    {
                        ...
                        // 设置中文提示文本
                        string cnToolTipText = childSection.GetStringValue("CNToolTip", null);
                        if (!string.IsNullOrWhiteSpace(cnToolTipText))
                        {
                            toolTipControl.CNText = cnToolTipText;
                        }
