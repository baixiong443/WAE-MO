...
namespace TSMapEditor.UI.CursorActions
{
    ...
    public class CheckDistanceCursorAction : CursorAction
    {
        ...
        public override string GetName() => TSMapEditor.UI.MainMenu.IsChinese ? "检查距离" : "Check Distance";
        ...
        public override void DrawPreview(Point2D cellCoords, Point2D cameraTopLeftPoint)
        {
            ...
            if (isChinese)
            {
                text = "路径长度（单元格）：" + pathLength + Environment.NewLine + 
                    "（X 差值：" + xDiff + "，Y 差值：" + yDiff + "）" + Environment.NewLine + Environment.NewLine + 
                    "点击选择新的源坐标点，或右键退出";
            }
            else
            {
                text = "Path Length In Cells: " + pathLength + Environment.NewLine + 
                    "(X Diff:" + xDiff + ", Y Diff: " + yDiff + ")" + Environment.NewLine + Environment.NewLine + 
