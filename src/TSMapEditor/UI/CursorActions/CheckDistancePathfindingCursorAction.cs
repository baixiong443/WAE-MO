...
namespace TSMapEditor.UI.CursorActions
{
    ...
    public class CheckDistancePathfindingCursorAction : CursorAction
    {
        ...
        public override string GetName() => TSMapEditor.UI.MainMenu.IsChinese ? "检查距离（寻路）" : "Check Distance (Path)";
        ...
        public override void DrawPreview(Point2D cellCoords, Point2D cameraTopLeftPoint)
        {
            ...
            if (source == null)
            {
                text = isChinese ? 
                    "点击选择源坐标点，或右键退出" : 
                    "Click to select source coordinate, or right-click to exit";
                DrawText(cellCoords, cameraTopLeftPoint, text, sourceColor);
                return;
            }

            string movementZoneText = "";
            switch (movementZone)
            {
                case MovementZone.Land:
                    movementZoneText = isChinese ? "陆地" : "Land";
                    break;
                case MovementZone.Water:
                    movementZoneText = isChinese ? "水域" : "Water";
                    break;
                case MovementZone.LandAndWater:
                    movementZoneText = isChinese ? "陆地和水域" : "Land and Water";
                    break;
                case MovementZone.Air:
                    movementZoneText = isChinese ? "空中" : "Air";
                    break;
            }
            ...
            if (isChinese)
            {
                instruction = Environment.NewLine + Environment.NewLine +
                    "当前模式：" + (isInfantry ? "步兵" : "载具") + "（按I键切换）" + Environment.NewLine + Environment.NewLine +
                    "当前移动区域：" + movementZoneText + "（按C键在陆地、水域和两者之间循环切换）";
            }
            else
            {
                instruction = Environment.NewLine + Environment.NewLine +
            ...
            {
                if (isChinese)
                    text = "路径长度（单元格）：" + pathCellCoords.Count + "\r\n\r\n点击选择新的源坐标点，或右键退出" + instruction;
                else
