...
namespace TSMapEditor.UI.CursorActions
{
    ...
    public class CopyCustomShapedTerrainCursorAction : CopyTerrainCursorActionBase
    {
        ...
        public override void DrawPreview(Point2D cellCoords, Point2D cameraTopLeftPoint)
        {
            ...
            {
                text = "按左键点击单元格标记为要复制的区域。" + Environment.NewLine + Environment.NewLine +
                      "按住SHIFT键点击可移除已选单元格。" + Environment.NewLine + Environment.NewLine +
                      "准备好后按ENTER键将单元格复制到剪贴板。";
            }
            else
            {
                text = "Press left click on cells to mark them to be copied." + Environment.NewLine + Environment.NewLine +
