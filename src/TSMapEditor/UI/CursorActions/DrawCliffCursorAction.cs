...
namespace TSMapEditor.UI.CursorActions
{
    ...
    public class DrawCliffCursorAction : CursorAction
    {
        ...
        {
            string mainText, tabText, pageUpDownText, exitText;
            
            if (TSMapEditor.UI.MainMenu.IsChinese)
            {
                mainText = "点击单元格放置一个新顶点。\r\n\r\n" +
                    "按ENTER键确认\r\n" +
                    "按Backspace键后退一步\r\n";
                tabText = "按TAB键在前侧和后侧之间切换\r\n";
                pageUpDownText = "按PageUp键抬高地形，按PageDown键降低地形\r\n";
                exitText = "按右键或ESC键退出";
            }
            else
            {
                mainText = "Click on a cell to place a new vertex.\r\n\r\n" +
                    "ENTER to confirm\r\n" +
                    "Backspace to go back one step\r\n";
                tabText = "TAB to toggle between front and back sides\r\n";
                pageUpDownText = "PageUp to raise the tiles, PageDown to lower them\r\n";
                exitText = "Right-click or ESC to exit";
            }
