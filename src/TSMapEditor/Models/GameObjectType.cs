...
namespace TSMapEditor.Models
{
    public abstract class GameObjectType : AbstractObject, INIDefined
    {
        ...
        public string GetEditorDisplayName()
        {
            ...
            // 优先用 TechnoNameManager 查找中文名
            if (MainMenu.IsChinese)
            {
                string chineseName = null;
                if (this is BuildingType)
                {
                    TechnoNameManager.TryGetBuildingName(ININame, out chineseName);
                }
                else if (this is AircraftType)
                {
                    TechnoNameManager.TryGetVehicleName(ININame, out chineseName);
                }
                else if (this is InfantryType)
                {
                    TechnoNameManager.TryGetInfantryName(ININame, out chineseName);
                }
                else if (this is UnitType)
                {
                    TechnoNameManager.TryGetVehicleName(ININame, out chineseName);
                }
