...
namespace TSMapEditor.Models
{
    ...
    public class TaskForce : IIDContainer, IHintable
    {
        ...
        public string GetHintText()
        {
            ...
            sb.Append(MainMenu.IsChinese ? "包含单位:" : "Contains:");
            ...
            foreach (var entry in TechnoTypes)
            {
                ...
                if (MainMenu.IsChinese)
                    sb.Append($"{entry.Count}个{entry.TechnoType.GetEditorDisplayName()}");
                else
                    sb.Append($"{entry.Count}x {entry.TechnoType.GetEditorDisplayName()}");
            }
