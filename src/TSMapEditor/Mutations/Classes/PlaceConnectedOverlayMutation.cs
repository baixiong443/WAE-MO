...
namespace TSMapEditor.Mutations.Classes
{
    ...
    class PlaceConnectedOverlayMutation : Mutation
    {
        ...
        public override string GetDisplayString()
        {
            ...
            string overlayName = connectedOverlayType.UIName;
            // 尝试获取中文名称
            if (isChinese)
            {
                var nameMap = TSMapEditor.UI.Sidebar.OverlayListPanel.GetConnectedOverlayChineseNameMap();
                if (nameMap.TryGetValue(overlayName, out var zhName))
                {
                    overlayName = zhName;
                }
            }
            ...
            if (isChinese)
            {
                return $"在 {cellCoords} 放置连接型覆盖 '{overlayName}'，笔刷大小 {brush}";
            }
            else
            {
                return $"Place connected overlay '{overlayName}' at {cellCoords} with a brush size of {brush}";
            }
        }
