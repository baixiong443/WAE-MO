...
namespace TSMapEditor.UI.TopBar
{
    class TopBarMenu : EditorPanel
    {
        ...
        public void RefreshLanguage(bool isChinese)
        {
            ...
            {
                int index = 0;
                if (toolsContextMenu.Items[index].Text.Contains("Options"))
                    toolsContextMenu.Items[index++].Text = isChinese ? "选项" : "Options";
                    
                toolsContextMenu.Items[index++].Text = isChinese ? "检查距离" : "Check Distance";
                toolsContextMenu.Items[index++].Text = isChinese ? "检查寻路距离" : "Check Pathfinding Distance";
                toolsContextMenu.Items[index++].Text = isChinese ? "计算矿石价值" : "Calculate Tiberium Value";
                toolsContextMenu.Items[index++].Text = " "; // 分隔符
                toolsContextMenu.Items[index++].Text = isChinese ? "地形生成器" : "Terrain Generator";
                ...
                if (index < toolsContextMenu.Items.Count && toolsContextMenu.Items[index].Text == " ")
                    toolsContextMenu.Items[index++].Text = " ";
            ...
            if (scriptingContextMenu != null && scriptingContextMenu.Items.Count > 0)
            {
                ...
                scriptingContextMenu.Items[index++].Text = isChinese ? "分组" : "Teams";
                scriptingContextMenu.Items[index++].Text = isChinese ? "触发" : "Triggers";
                scriptingContextMenu.Items[index++].Text = isChinese ? "标签" : "Tags";
                scriptingContextMenu.Items[index++].Text = isChinese ? "本地变量" : "Local Variables";
                scriptingContextMenu.Items[index++].Text = " "; // 分隔符
                
                if (index < scriptingContextMenu.Items.Count)
                    scriptingContextMenu.Items[index++].Text = isChinese ? "任务" : "Mission";
                if (index < scriptingContextMenu.Items.Count)
                    scriptingContextMenu.Items[index++].Text = isChinese ? "教学" : "Tutorial";
                if (index < scriptingContextMenu.Items.Count)
                    scriptingContextMenu.Items[index++].Text = " "; // 分隔符
                if (index < scriptingContextMenu.Items.Count)
                    scriptingContextMenu.Items[index++].Text = isChinese ? "AI触发" : "AITriggers";
            }
