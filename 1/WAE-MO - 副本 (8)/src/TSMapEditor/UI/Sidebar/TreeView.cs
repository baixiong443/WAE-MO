        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            for (int i = 0; i < Categories.Count; i++)
            {
                ...
                if (ViewTop < height + CategoryHeight)
                {
                    ...
                    string text = null;
                    string display = category.DisplayName ?? category.Text ?? string.Empty;
                    if (category.Nodes.Count > 0)
                        text = category.IsOpened ? "- " + display : "+ " + display;
                    else
                        text = display;
                ...
                if (Categories[i].IsOpened)
                {
                    for (int j = 0; j < category.Nodes.Count; j++)
                    {
                        ...
                        if (ViewTop < height + LineHeight)
                        {
                            ...
                            int textHeight = (int)Renderer.GetTextDimensions(node.DisplayName ?? node.Text ?? string.Empty, Constants.UIDefaultFont).Y;
                            int textY = y + ((LineHeight - textHeight) / 2);
                            DrawStringWithShadow(node.DisplayName ?? node.Text ?? string.Empty, Constants.UIDefaultFont, new Vector2(x, textY), UISettings.ActiveSettings.AltColor);
